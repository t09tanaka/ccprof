...
慢性的なコマンドコストを表示します。同じ `finding_key` が2つ以上の解析に
出現した場合は Recurring findings として、初出時と最新の回収見積もりの
推移（improved / worsened / flat）を表示し、提案採用後の効果を追跡でき
ます。
...
を使います。1つのセッションが複数のブランチを観測している場合は、head
branch 上で記録された区間（branch 情報の無い行は直前のブランチに帰属）
だけを計上し、他ブランチでの作業時間や編集を現 PR に混ぜません。
