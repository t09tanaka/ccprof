...
The program addresses the remaining audit hardening work. The following are already fixed and are not redesigned or scheduled here: transcript budget handling, Store v2 basics, v0.3 version alignment, and SBOM/build attestations. Remaining supply-chain work includes npm Trusted Publishing/OIDC, npm provenance, and reproducible release verification. Existing users and stored audit data remain supported through additive schemas, readers, and migration tooling.
...
The Store is extended with source descriptor, run budget, catalog, and history-query tables. `AnalysisBudgets` records exactly `max_input_bytes`, `max_input_events`, `max_wall_ms`, `max_cpu_ms`, `max_output_bytes`, and `max_source_items`, and reports partial/truncation coverage when any limit is reached. Source descriptors preserve provenance and retention classification; the incremental `source_catalog` records adapter/version, source identity, content revision, discovery cursor, observed time, and completeness. The catalog indexes rules, report schemas, source kinds, and exports. History-query tables provide bounded, indexed summaries without requiring transcript replay; no new rule-decision ledger table is added.

Privacy has one mandatory projection used by analyze, stats, explain, diagnose, export, warnings, errors, and advisory input. `stats --privacy strict --json` always applies that projection. CI and collection run in strict mode and cannot be weakened. A stats privacy projection removes disallowed fields before terminal aggregation keyed by the distinct repository, PR, and terminal snapshot. Interval unions are deduplicated before metrics are calculated. Snapshots separately report `confirmed_critical_path_ms`, `estimated_critical_path_upper_ms`, `resource_cost_ms`, `human_wait_ms`, and `unexplained_ms`. Cohorts must meet a minimum threshold before aggregate output is emitted; smaller cohorts are suppressed. Comparable baseline and R006 cohorts require the same repository/workspace, similar changed-file count and diff size, the same command identity, and observable cache state. Baselines include median, p50, p75, MAD, and sample count. Stats use bounded dimensions, redacted identifiers, and explicit privacy metadata. Raw content is never introduced into aggregate tables.

Organization policy is signed and schema-validated with `minimum_privacy`, `allow_raw`, `allow_advisory`, `allow_export`, `raw_retention_days_max`, and `required_source_coverage`. Precedence is organization constraints over repository policy over CLI settings; lower layers can only tighten controls. An administrative advisory kill switch disables advisory execution regardless of lower-layer settings. Policy controls define separate `raw_evidence_retention_days` and `aggregate_retention_days`, `max_bytes`/quota, storage mode (`os_keychain`, `enterprise_key`, or `none`), platform key providers, deletion behavior, and export authorization. Logical repository IDs resolve by configured provider order and remain distinct from local filesystem IDs; workspace adapters define the workspace model and policy scope. The audit CLI family includes doctor, diagnose, policy, config, schema, export, import, store verify/backup/restore, source, and rules. Data-governance policy prohibits use of audit data for employee performance assessment, surveillance, or unrelated model training. The doctor command verifies configuration, migrations, capabilities, encryption configuration, budget enforcement, and store health.
...
5. Prove behavior and release: Node 22/24 Linux/macOS/Windows matrix, Node 26 canary/native-addon coverage, calibration/property/fuzz/fault/performance gates with documented fixtures and SLOs, npm Trusted Publishing/OIDC, npm provenance, reproducible release verification, and operator/support documentation. npm registry/organization enablement is an external prerequisite.
